	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'philip': //week 1 stage 1
				...
				var stageFront:BGSprite = new BGSprite('philip/stage1', -650, 265, 0.9, 0.9);
				stageFront.setGraphicSize(Std.int(stageFront.width * 1.0));
				stageFront.updateHitbox();
				add(stageFront);

				var micw:BGSprite = new BGSprite('philip/mic/mic_with_weapon', 385, 0, 1, 1);
				micw.updateHitbox();
				add(micw);

			case 'tests':

				var bg:BGSprite = new BGSprite('test/CatBG', -550, -400, 0.5, 0.5);
				bg.setGraphicSize(Std.int(bg.width * 0.8));
				add(bg);

				eye = new BGSprite('test/cateye', 0, -200, 0.5, 0.5, ['catbg idle']);
				eye.setGraphicSize(Std.int(eye.width * 0.8));
				eye.updateHitbox();
				add(eye);

				var stageFront:BGSprite = new BGSprite('test/CatFloor', -230, -200, 0.5, 0.5);
				stageFront.setGraphicSize(Std.int(stageFront.width * 0.7));
				stageFront.updateHitbox();
				add(stageFront);
			...
			case 'tech':
				...
				var bg:BGSprite = new BGSprite('tech/bg', -600, -400, 0.9, 0.9);
				bg.setGraphicSize(Std.int(bg.width * 0.8));
				add(bg);

				var bg:BGSprite = new BGSprite('tech/3', -600, -400, 0.9, 0.9);
				bg.setGraphicSize(Std.int(bg.width * 0.8));
				add(bg);

				var bg:BGSprite = new BGSprite('tech/2', -600, -400, 0.9, 0.9);
				bg.setGraphicSize(Std.int(bg.width * 0.8));
				add(bg);

				var bg:BGSprite = new BGSprite('tech/1', -600, -400, 0.9, 0.9);
				bg.setGraphicSize(Std.int(bg.width * 0.8));
				add(bg);

				th1 = new BGSprite('tech/tn1', 0, -200, 0.5, 0.5, ['tn1 idle']);
				th1.setGraphicSize(Std.int(th1.width * 0.8));
				th1.updateHitbox();
				add(th1);

				th2 = new BGSprite('tech/tn2', 0, -200, 0.5, 0.5, ['tn2 idle']);
				th2.setGraphicSize(Std.int(th2.width * 0.8));
				th2.updateHitbox();
				add(th2);
				...
				var stageFront:BGSprite = new BGSprite('tech/floor', -500, 0, 0.9, 0.9);
